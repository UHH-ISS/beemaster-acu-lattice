...
struct pattern {
        ...
        //pattern(const pattern& p) : srcIp(p.srcIp), srcPrt(p.srcPrt), dstPrt(p.dstPrt), protocol(p.protocol), count(p.count), support(p.support) {
        //}
    };
...
namespace std{
    template<> struct hash<pattern> {
        std::size_t operator()(const pattern& k) const{
            ...
            // and bit shifting:
            return (hash<string>()(k.signature));
...
namespace acu{
    ...
    class LatticeCorrelation {
            ...
            pattern generatePattern(alert a, string patternSignature, int alertsSize){
                ...
                //p.type;
                //printf("sig: %s \n", patternSignature.c_str());
                //printf("pattern: %s, %d, %d, %s \n", p.srcIp.c_str(), p.srcPrt, p.dstPrt, p.protocol.c_str());
        ...
        public:
            ...
            unordered_set<pattern> correlate(vector<alert> alerts){
                ...
                unordered_set<pattern> patterns_ip;
                    for(std::size_t i = 0; i < alerts.size(); i++){
                        ...
                        for(string patternType : this->patternsTypes){
                            printf("generatePattern...\n");
                            lattice_ip.insert(generatePattern(currAlert, patternType, alerts.size())) ;
                            printf("lattice_ip(%d): %d\n", &lattice_ip, lattice_ip.size());
                        }
                        //printf
                        lattice[ip] = lattice_ip;
                    }
                ...
                for(auto& lattice_ip1 : lattice){
                    //printf("lattice_ip.addr: %d\n", &lattice_ip1->second);
