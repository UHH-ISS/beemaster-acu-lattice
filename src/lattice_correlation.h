    ...
    public:
        ...
        LatticeCorrelation(beemaster::VectorStorage *vStorage, beemaster::RocksStorage<int> *storage,
            ...
            this->attackMap = {{1, "scan"},
                               ...
                               {4, "worm"},
                               {5, "reflector DDoS"},
                               {6, "SYN flood response"},
                               {7, "W32/Blast worm"},
                               {8, "SQL-Slammer worm"}};
        };
        std::unordered_map<std::string, pattern *> *
        correlate(std::vector<const acu::IncomingAlert *> alerts, float threshold);
        std::unordered_map<std::string, pattern *> *
        latticeCompression(std::unordered_map<std::string, pattern *> *lattice, float threshold);
